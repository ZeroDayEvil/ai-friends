...
В командном режиме без политики доступ закрыт полностью, поэтому файл нужно
создать до выдачи доступа сотрудникам:

```bash
cp infra/vps/machine-policy.sample.json \
   /var/lib/docker/volumes/electerm-platform_app-data/_data/machine-policy.json
```

Проще и надёжнее — положить файл через контейнер, чтобы не зависеть от
внутреннего устройства томов:

```bash
docker compose cp machine-policy.json app:/app/data/machine-policy.json
```

Адреса почты должны совпадать с теми, что выдаёт Access. Файл перечитывается при
изменении, перезапуск не нужен: он рвал бы чужие сессии.
