...
Отдельно стоит назвать вид дефекта, которому здесь уделено непропорционально
много места. Это не «нет проверки», а «проверка смотрит не туда»: правило
читает объявленное вызывающим слово вместо самого значения, сверяет одно поле из
пяти связанных, доверяет усечённому хешу или сравнивает строку вместо разбора.
Такой дефект не виден при чтении: правило в тексте выглядит выполненным. Ниже
он проходит как T-28 — T-37 и покрыт отвергаемыми примерами.
...
| T-28 | Расхождение вида субъекта и его идентификатора | N7, ошибка реализации | одна часть системы считает запись человеком, другая — агентом | `principal_ref` связывает `kind` с префиксом `id` | A |
| T-29 | Исполнительное полномочие переживает наблюдателя | N5, N7 | действие выполняется после конца смены, без надзора | `af_supervisor_sid` обязателен и для `af-exec`, проверка активности сессии при выполнении | A |
| T-30 | Разнесённые привязки одобрения проверяются по одной | N7, ошибка реализации | пропуск одной сверки превращает одобрение безобидного вызова в право на опасный | `execution_binding` под одной подписью, сверка полей инвариантом | A |
| T-31 | Заявленное происхождение аргументов | N7 | агент объявляет недоверенные данные доверенными и обходит одобрение | подпись оркестратора и принудительный подъём признака по составу источников | A |
| T-32 | Имя устройства в надстрочной записи | N5, N3 | `COM¹` доходит до открытия как имя устройства | перечисление надстрочных форм в `relpath`, проверка типа объекта у дескриптора | A |
| T-33 | Подбор усечённого хеша политики | N5, N6 | «версия 3.4.1 с таким-то содержимым» перестаёт указывать на содержимое | полный SHA-256 плюс подпись службы политики | A |
| T-34 | Объявленный класс сетевого адреса | N7, N2 | публичный адрес объявляется петлевым, служебный — публичным | класс определяет служба, соответствие проверяется разбором адреса | A |
| T-35 | Рассогласование протокола подключения | N7 | проверка читает протокол цели, а соединение идёт по внешнему полю | протокол объявляется один раз внутри `target`; привязка брокера сверяется | A |
| T-36 | Загрязнение прототипа через поля документа | N2, N7 | отсутствующее поле выглядит существующим, проверка обходится | доступ только к собственным свойствам, запрет имён `__proto__`, `constructor`, `prototype` | A |
| T-37 | Несуществующая отметка времени | N7, N3 | запись, которую нельзя поместить на ось времени журнала | разбор с проверкой календаря вместо сопоставления с образцом | A |
...
| A-56 | Запись субъекта с видом `human` и идентификатором `agent:` | отвергается схемой |
| A-57 | Профиль агента с релизом A и `unattended_allowed` | отвергается схемой |
| A-58 | Токен `af-exec` в релизе A без `af_supervisor_sid` | отвергается схемой |
| A-59 | Сессия наблюдателя завершена между выдачей `af-exec` и выполнением | выполнение отклонено |
| A-60 | `af_binding` ссылается на другой запрос или другое лицо | отвергается инвариантом |
...
| A-61 | Одобрение без `request_id` | отвергается схемой |
| A-62 | `ttl_seconds` одобрения не совпадает с окном | отвергается инвариантом |
| A-63 | Версия политики без полного хеша или без подписи | отвергается схемой |
...
| A-64 | Недоверенный источник в списке при `derived_from_untrusted: false` | отвергается схемой |
| A-65 | Блок происхождения без подписи оркестратора | отвергается схемой |
| A-66 | Поле `__proto__` или `constructor` в свободной части аргументов | отвергается до проверки схемы |
| A-67 | Отметка времени `2026-02-30` или час `25` | отвергается разбором времени |
...
| A-68 | `COM¹`, `LPT²`, `CONIN$` в относительном пути | отвергается схемой |
| A-69 | `COMET.txt` и `COMMON` в относительном пути | принимаются: правило не должно быть шире нужного |
| A-70 | Дескриптор указывает на устройство или канал, а не на файл | отказ `containment_violation` |
| A-71 | Публичный адрес публикации объявлен петлевым | отвергается инвариантом |
| A-72 | Срок публикации не совпадает с парой отметок времени | отвергается инвариантом |
| A-73 | Разрешение брокера выдано на другую цель, профиль или протокол | отвергается инвариантом |
...
| A-74 | Запись субъекта исполнителя с `can_write`, включающим политику или журнал | отвергается схемой |
| A-75 | Запись субъекта исполнителя без снятой `SeDebugPrivilege` | отвергается схемой |
...
| A-55 | Обращение к `169.254.169.254` | отказ с указанием класса |
| A-76 | Разрешение исходящего соединения в обход прокси | отвергается схемой |
| A-77 | Общее разрешение с непубличным адресом без узкого исключения | отвергается схемой |
| A-78 | Служебный адрес объявлен публичным | отвергается инвариантом |
| A-79 | Узкое исключение под службу метаданных | отвергается схемой |

Проверки A-01 — A-04, A-13, A-14, A-24 — A-26, A-30 — A-34, A-54, A-56 — A-58,
A-61, A-63 — A-69, A-71 — A-79 выполняются командой
`node security/schemas/validate.mjs`: соответствующие случаи лежат в
...
Что примеры действительно проверяют, а не сопровождают текст, проверяется
инверсией: если перевернуть поле `valid` у всех записей `examples/index.json`,
прогон обязан выдать столько же жалоб, сколько записей. Совпадение означает, что
вердикт каждого примера определён проверкой, а не совпадением.
