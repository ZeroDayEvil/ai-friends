...
#>
[CmdletBinding()]
# PSScriptAnalyzer считает ошибкой создание SecureString из открытого текста, и
# по существу он прав. Здесь это неизбежно: скрипт сам генерирует пароль для
# новой учётной записи и обязан передать его человеку, иначе войти будет некому.
# Открытый текст живёт в памяти до записи в файл, доступный только
# администраторам и SYSTEM, и сценарий предписывает удалить файл после передачи.
# Обход правила через посимвольный AppendChar дал бы ту же плоскую строку в
# памяти, но спрятал бы её от анализатора, поэтому выбрано явное подавление.
[Diagnostics.CodeAnalysis.SuppressMessageAttribute(
    'PSAvoidUsingConvertToSecureStringWithPlainText', '',
    Justification = 'Сгенерированный пароль однократно передаётся владельцу через защищённый файл.')]
param(
    [string[]]$CreateUsers,
    [switch]$EnsureSsh,
